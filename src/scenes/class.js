...
this.cursors = this.input.keyboard.createCursorKeys();
    ...
    this.add.image(game.config.width/4 + 360 , game.config.height/4 - 150, 'bar4', menuConfig).setScale(0.6).setOrigin(0)
    //
    
	this.label = this.add.text(80, 260, 'Even though this may hurt your friendship, you stay out of the fight. ' ,menuConfig).setInteractive();
    this.label = this.add.text(80, 280,'You head to your next class after lunch.',menuConfig).setInteractive();
    this.label = this.add.text(80, 300, 'In the middle of class an officer comes in for a surprise bag check.',menuConfig).setInteractive();
    this.label = this.add.text(80, 320, 'The dogs sniff about the class room, skipping over you. However,',menuConfig).setInteractive();
    this.label = this.add.text(80, 340, 'the officer still asks you to hand over your bag.',menuConfig).setInteractive();
   ...
   // this.typewriteText('You wake up in the morning to realize your alarm clock did not go off.\n You\'re late for schoool! Filled with panic, you rush out to get to class before security closes the school gate..');
    ...
    this.typewriteText2('Hand them your bag');
    ...
    this.typewriteText3('Insist this is unfair');
